set-location D:\Git\DeepNestSharp
...
cd ..
Compress-Archive -Path DeepNestSharp\bin\Release\net5.0-windows\win-x64\publish\* -DestinationPath "DeepNestSharp\bin\Release\DeepNestSharp.x64-v${semVer}.zip" -Force
& dotnet restore -r win-x86
& dotnet build -c Release
cd DeepNestSharp
& dotnet publish -r win-x86 -c Release --self-contained #-p:PublishSingleFile=true
cd ..
Compress-Archive -Path DeepNestSharp\bin\Release\net5.0-windows\win-x86\publish\* -DestinationPath "DeepNestSharp\bin\Release\DeepNestSharp.x86-v${semVer}.zip" -Force